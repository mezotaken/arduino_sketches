...
bool teq(const TimePoint &t1, const TimePoint &t2) {
    return t1.hour == t2.hour && t1.minute == t2.minute;
}
bool tge(const TimePoint &t1, const TimePoint &t2) {
...
const byte closed[n] = {5, 0, 22, 10};
const byte opened[n] = {110, 85, 100, 100};
...
//------------------------Logic functions------------------//
byte find_closest_index(const TimePoint &cur) {
    byte i = 0;
    while (i < n && tge(cur, schedule[i]))
        i++;
    return i % n;
}
